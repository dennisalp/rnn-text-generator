...
def ids2text(ids2chars, ids):
    return tf.strings.reduce_join(ids2chars(ids), axis=-1)
...
def main():
    ...
    model = MyModel(vocab_size=len(chars2ids.get_vocabulary()), embedding_dim=embedding_dim, rnn_units=rnn_units)
    ...
    one_step_model = OneStep(model, ids2chars, chars2ids)
